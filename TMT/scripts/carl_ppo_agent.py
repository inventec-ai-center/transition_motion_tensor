...
class CarlPPOAgent(PPOAgent):
    ...
    ACTOR_PRIMITIVES_NET_Hidden_Size_KEY = 'ActorPrimitivesNetHiddenSize'
    ACTOR_GATING_NET_KEY = 'ActorGatingNet'
    ACTOR_OPTIMIZER_KEY = 'ActorOptimizer'
    Critic_OPTIMIZER_KEY = 'CriticOptimizer'
    ...
    def __init__(self, world, id, json_data, seed):
        ...
        self.motion_names = []
        self.actor_primitive_sizes = [4]
        self.actor_primitive_net_hidden_size = 256
        self.enable_l1_regularization = False
    ...
    def _load_params(self, json_data):
        ...
        if self.PRIMITIVES_KEY in json_data:
            self.actor_primitive_sizes = json_data[self.PRIMITIVES_KEY]

        if self.ACTOR_PRIMITIVES_NET_Hidden_Size_KEY in json_data:
            self.actor_primitive_net_hidden_size = json_data[self.ACTOR_PRIMITIVES_NET_Hidden_Size_KEY]

        if self.GATING_REGULARIZATION_LAMBDA_KEY in json_data:
            self.enable_l1_regularization = json_data[self.GATING_REGULARIZATION_LAMBDA_KEY] > 0
        return
    ...
    def _build_nets(self, json_data):
        ...
        self.gating_mask_tf = tf.placeholder(tf.float32, shape=[None, np.sum(self.actor_primitive_sizes)])

        with tf.variable_scope('main'):
            with tf.variable_scope('actor'):
                self.a_mean_tf, self.a_std_tf = self._build_net_actor(primitives_net_name=actor_primitives_net_name,
                                                                      gating_net_name=actor_gating_net_name,
                                                                      primitive_sizes=self.actor_primitive_sizes,
            ...
            if self.enable_l1_regularization:
                with tf.variable_scope('generator'):
                    self._build_net_gating(net_name=actor_gating_net_name,
                                           primitive_sizes=self.actor_primitive_sizes,
                                           motion_names=self.motion_names,
                                           init_output_scale=actor_init_output_scale)
    ...
    def _build_losses(self, json_data):
        ...
        self.regularization_loss_tf = 0
        Logger.print('---------------------------- L1 Regularization ----------------------------')
        for i in range(0, len(vars_gating)):
            l1_loss = tf.reduce_mean(tf.keras.losses.MAE(vars_generator[i], vars_gating[i]))
            self.regularization_loss_tf += l1_loss
            Logger.print('{:<70} -> {:<70}'.format(vars_generator[i].name, vars_gating[i].name))
        Logger.print('---------------------------------------------------------------------------')
        self.actor_loss_tf += self.regularization_loss_tf * gating_regularization_lambda
    ...
    def _build_solvers(self, json_data):
        assert self.ACTOR_OPTIMIZER_KEY in json_data
        assert self.Critic_OPTIMIZER_KEY in json_data

        actor_optimizer = json_data[self.ACTOR_OPTIMIZER_KEY]
        ...
        critic_optimizer = json_data[self.Critic_OPTIMIZER_KEY]
        ...
        if critic_optimizer.lower() == "adam":
            critic_opt = tf.train.AdamOptimizer(learning_rate=critic_stepsize)
        elif critic_optimizer.lower() == "momentum":
        ...
        if not self.enable_pretraining: # Freeze the primitives paramters based on the training stage
            vars_to_remove = []
        ...
        if actor_optimizer.lower() == "adam":
            actor_opt = tf.train.AdamOptimizer(learning_rate=self._actor_stepsize_tf)
        elif actor_optimizer.lower() == "momentum":
    ...
    def _build_net_actor(self, primitives_net_name, gating_net_name, primitive_sizes, motion_names, init_output_scale):
        with tf.variable_scope('primitives'):
            primitives_mean_tf, primitives_std_tf = self._build_net_primitives(primitives_net_name, primitive_sizes, motion_names, init_output_scale)
        with tf.variable_scope('gating'):
            w_tf = self._build_net_gating(gating_net_name, primitive_sizes, motion_names, init_output_scale)

        # Apply weight masking / switching
        w_tf = tf.concat(w_tf, axis=1)  # Concatenate each weight to get (?, num_primitives)
        w_tf = w_tf * self.gating_mask_tf
    ...
    def _build_net_primitives(self, net_name, primitive_sizes, motion_names, init_output_scale):
        ...
        primitives_mean_tfs = []
        primitives_std_tfs = []
        for name, num_primitives in zip(motion_names, primitive_sizes):
            with tf.variable_scope(name):
                ...
                for i in range(0, num_primitives):
                    h2 = tf.layers.dense(inputs=h, units=self.actor_primitive_net_hidden_size, activation=tf.nn.relu, name='primitive_%d_dense' % (i),
                    ...
                    primitives_mean_tfs.append(norm_mean_tf)
                    primitives_std_tfs.append(norm_std_tf)
        return primitives_mean_tfs, primitives_std_tfs

    def _build_net_gating(self, net_name, primitive_sizes, motion_names, init_output_scale):
        ...
        # Initialize each motion's gating network
        w_tfs = []
        for name, num_primitives in zip(motion_names, primitive_sizes):
            with tf.variable_scope(name):
                # Intermediate layer
                ...
                # Final layer
                w_tf = tf.layers.dense(inputs=h, units=num_primitives, activation=tf.nn.sigmoid, kernel_initializer=TFUtil.xavier_initializer)
                w_tfs.append(w_tf)
        return w_tfs
    ...
    def _build_gating_mask(self, motion_labels, primitives):
        one_hot_labels = np.eye(len(primitives))[motion_labels.ravel()]
        gating_mask = np.repeat(one_hot_labels, repeats=primitives, axis=1)
        return gating_mask
    ...
    def _update_actor(self, s, g, a, logp, adv):
        feed = {
            ...
            self.gating_mask_tf: np.ones((s.shape[0], self.gating_mask_tf.shape[1]), dtyp=np.float32),
        }
    ...
    def _eval_actor(self, s, g, enable_exp):
        ...
        motion_label = np.reshape(motion_label, [-1, 1])
        gating_mask = self._build_gating_mask(motion_labels=motion_label, primitives=self.actor_primitive_sizes)

        feed = {
            self.s_tf : s,
            self.g_tf : g,
            self.exp_mask_tf: np.array([1 if enable_exp else 0]),
            self.gating_mask_tf: gating_mask,
        }
    ...
    def load_model(self, policy_model_path):
        with self.sess.as_default(), self.graph.as_default():
            try:
                self.saver.restore(self.sess, policy_model_path)
            except:
                Logger.print('Failed to load policy model from: ' + policy_model_path)
                assert(False)
    ...
    def load_primitive_model(self, motion_name, model_path):
        with self.sess.as_default(), self.graph.as_default():
            Logger.print("==================")
            Logger.print("Loading primitives (Motion: %s, Model: %s)" % (motion_name, model_path))
            Logger.print("==================")

            reader = pywrap_tensorflow.NewCheckpointReader(model_path)
            var_to_shape_map = reader.get_variable_to_shape_map()
            var_list = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='agent/main/actor/primitives/{}'.format(motion_name))

            for v in var_list:
                naming_0 = v.name.replace("/{}".format(motion_name), "")[:-2] # case 1:
                naming_1 = v.name[:-2]                                        # case 2:

                lookfor = ''
                for item in var_to_shape_map:
                    if item == naming_0:
                        lookfor = naming_0
                        break
                    elif item == naming_1:
                ...
                if lookfor == '':
                    Logger.print("Variable {} not found".format(v.name))
                    assert(False)

                weights = reader.get_tensor(lookfor)
                self.sess.run(v.assign(weights))
                Logger.print("Assign {} {} --> {} {}".format(lookfor, weights.shape, v.name, v.shape))
        return

    def load_gating_model(self, motion_name, model_path):
        with self.sess.as_default(), self.graph.as_default():
            Logger.print("==================")
            Logger.print("Loading gating (Motion: %s, Model: %s)" % (motion_name, model_path))
            Logger.print("==================")

            reader = pywrap_tensorflow.NewCheckpointReader(model_path)
            var_to_shape_map = reader.get_variable_to_shape_map()
            var_list = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='agent/main/actor/gating/{}'.format(motion_name))

            for v in var_list:
                naming_0 = v.name.replace("/{}".format(motion_name), "")[:-2]
                naming_1 = v.name[:-2]
                naming_2 = v.name.replace("agent/main/actor/gating/{}".format(motion_name),"generator")[:-2]
                if "generator/dense" in naming_2:
                    naming_2 = naming_2.replace("generator/dense","generator/5/dense")
                ...
                weights = reader.get_tensor(lookfor)
                goal_fc = "agent/main/actor/gating/{}/2/dense/kernel".format(motion_name)
                if goal_fc in v.name and weights.shape[0] + 1 == v.shape[0]:
                    padding = np.zeros([1, weights.shape[1]])
                    weights = np.concatenate([weights,padding], axis=0)
                    self.sess.run(v.assign(weights))
                else:
                    self.sess.run(v.assign(weights))
                Logger.print("Assign {} {} --> {} {}".format(lookfor, weights.shape, v.name, v.shape))
            ...
            # if self.enable_l1_regularization:
            #     Logger.print("==============")
            #     Logger.print("Copying params generator (Motion: %s)" % (motion_name))
            #     Logger.print("==============")

            #     generator_var_list = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='agent/main/generator/{}'.format(motion_name))
            #     for v in generator_var_list:
            #         lookfor = v.name.replace("generator", "actor/gating")

            #         Logger.print("Assign {} --> {}".format(lookfor, v.name))
            #         for gating_v in var_list:
            #             if gating_v.name == lookfor:
            #                 self.sess.run(v.assign(gating_v.eval()))
        return

    def load_normalizer(self, normalier_path):
        with self.sess.as_default(), self.graph.as_default():
            data = np.load(normalier_path)
            state_mean = data['states_mean']
            state_std = data['states_std']
            goal_mean = data['states_ref_mean']
            goal_std = data['states_ref_std']

            self.s_norm.set_mean_std(state_mean, state_std)
            if goal_mean.shape == self.g_norm.mean.shape and goal_std.shape == self.g_norm.std.shape:
                self.g_norm.set_mean_std(goal_mean, goal_std)
            else:
                Logger.print('Skip loading goal normalizer due to shape mismatch')

            self.s_norm.update()
            self.g_norm.update()
            self._load_normalizers()
            Logger.print('Normalizer loaded from: ' + normalier_path)

        Logger.print('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
        Logger.print(' State %f, %f' % (np.mean(self.s_norm.mean), np.mean(self.s_norm.std)))
        Logger.print('  Goal %f, %f' % (np.mean(self.g_norm.mean), np.mean(self.g_norm.std)))
        Logger.print('Action %f, %f' % (np.mean(self.a_norm.mean), np.mean(self.a_norm.std)))
        Logger.print(' Value %f, %f' % (np.mean(self.val_norm.mean), np.mean(self.val_norm.std)))
        Logger.print('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
        return
