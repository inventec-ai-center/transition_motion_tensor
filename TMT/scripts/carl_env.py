import numpy as np
from CarlCore import CarlCore
from env.env import Env
from env.action_space import ActionSpace

class CarlEnv(Env):
    def __init__(self, args, arg_parser, enable_draw):
        super().__init__(args, enable_draw)

        self._core = CarlCore.cCarlCore(enable_draw)

        rand_seed = arg_parser.parse_int('rand_seed', -1)
        if rand_seed < 0:
    ...
    def record_phase(self, agent_id):
        return self._core.RecordPhase(agent_id)

    def record_motion_label(self, agent_id):
        return self._core.RecordMotionLabel(agent_id)

    def record_goal_target(self, agent_id):
        return np.array(self._core.RecordGoalTarget(agent_id))
    ...
    def switch_motion(self, agent_id, target_motion, target_phase):
        return self._core.SwitchMotion(agent_id, target_motion, target_phase)
    ...
    def log_gating_weights(self, agent_id, weights):
        self._core.LogGatingWeights(agent_id, weights.tolist())
        return

    def log_primitives_means_stds(self, agent_id, num_primitives, means, stds):
        self._core.LogPrimitivesMeanStd(agent_id, num_primitives, means, stds)
        return
